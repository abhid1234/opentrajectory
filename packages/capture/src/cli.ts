...
function main(): void {
  ...
  if (cmd === "to-otel") {
    const input = rest.find((a) => !a.startsWith("-"));
    if (!input) return fail("usage: ot to-otel <file.ot.json> [-o out.json]   (OTLP/JSON for any OTel collector)");
    const out = arg(["-o", "--out"], rest);
    const traces = loadDocs(input).map((d) => toOtel(d as Trajectory));

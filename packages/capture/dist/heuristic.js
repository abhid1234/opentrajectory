...
function stepText(s) {
    if (s.tool_call)
        return [s.tool_call.result, s.tool_call.error, JSON.stringify(s.tool_call.args)].filter(Boolean).join("\n");
    if (s.message)
 ...
 */
export function diagnoseHeuristic(traj) {
    const evidence = [];
    ...
    for (const s of traj.steps) {
        ...
        if (CTX_RE.some((re) => re.test(text)) && (failed || s.role === "tool")) {
            ...
            if (offending == null)
                offending = s.index;
            if (ctxHits <= 3)
                evidence.push("missing-context marker: " + firstLine(s.tool_call?.error || s.tool_call?.result || text));
        }
    }
    ...
    for (const s of traj.steps) {
        if (!s.tool_call)
            continue;
        const path = String(s.tool_call.args?.path ?? s.tool_call.args?.file_path ?? "");
    ...
    if (resolved && !harness && !training) {
        diagnosis = "CLEAN";
        category = "Clean";
        confidence = 0.9;
    }
    else if (harness && training) {
        diagnosis = "BOTH";
        ...
        confidence = Math.min(0.5 + 0.15 * ctxHits, 0.95);
    }
    else if (training) {
        diagnosis = "TRAINING";
        category = "Reward Hack";

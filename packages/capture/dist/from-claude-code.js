...
const SECRET_RE = /(sk-[A-Za-z0-9]{16,}|gh[pousr]_[A-Za-z0-9]{20,}|AIza[0-9A-Za-z_\-]{20,}|-----BEGIN [A-Z ]*PRIVATE KEY-----[\s\S]*?-----END [A-Z ]*PRIVATE KEY-----)/g;
function redact(s) {
    let redacted = false;
 ...
 */
export function fromClaudeCode(events, opts = {}) {
    ...
    if (toolSteps.length > 0) {
        if (lastTool.tool_call.success === false)
            status = "failure";
        else
...
export function captureFromTranscript(jsonl, opts) {

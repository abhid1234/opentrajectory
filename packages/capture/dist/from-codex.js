...
export function parseRollout(jsonl) {
    const out = [];
 ...
 */
export function fromCodex(events, opts = {}) {
    ...
    const steps = [];
    const pushStep = (s) => steps.push({ index: steps.length, ...s });
    let cliVersion;
    let sessionId;
    let cwd;
    let cleanUserPrompt; // from event_msg user_message (no preamble)
    let startedAt;
    let endedAt;
    for (const ev of events) {
        const p = ev?.payload;
...
export function looksLikeCodex(jsonl) {
    for (const line of jsonl.split("\n").slice(0, 40)) {
        ...
        try {
            ...
            if (["session_meta", "response_item", "event_msg", "turn_context"].includes(o.type))
                return true;

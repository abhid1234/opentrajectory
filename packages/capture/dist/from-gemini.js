...
function geminiToolResult(result) {
    if (!Array.isArray(result))
        return { text: asText(result), success: true };
    const parts = [];
    let success = true;
...
export function fromGemini(session, opts = {}) {
    ...
    for (const m of messages) {
        if (!m || typeof m !== "object")
            continue;
        if (m.type === "info")
            continue; // UI/system notices — not part of the trajectory
        const tok = m.tokens;
        if (tok) {
